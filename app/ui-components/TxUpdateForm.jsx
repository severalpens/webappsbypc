...
import { getTx } from "./graphql/queries";
import { updateTx } from "./graphql/mutations";
const client = generateClient();
export default function TxUpdateForm(props) {
  const {
    id: idProp,
    tx: txModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = txRecord
      ? { ...initialValues, ...txRecord }
      : initialValues;
    setTxDate(cleanValues.TxDate);
    setTxDateTime(cleanValues.TxDateTime);
    setTxDateDate(cleanValues.TxDateDate);
    setTxAmount(cleanValues.TxAmount);
    setTxType(cleanValues.TxType);
    setTxCategory(cleanValues.TxCategory);
    setTxDescription(cleanValues.TxDescription);
    setErrors({});
  };
  const [txRecord, setTxRecord] = React.useState(txModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTx.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTx
        : txModelProp;
  ...
  React.useEffect(resetStateValues, [txRecord]);
  const validations = {
    TxDate: [],
    TxDateTime: [],
    TxDateDate: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          TxDate: TxDate ?? null,
          TxDateTime: TxDateTime ?? null,
          TxDateDate: TxDateDate ?? null,
          TxAmount: TxAmount ?? null,
          TxType: TxType ?? null,
          TxCategory: TxCategory ?? null,
          TxDescription: TxDescription ?? null,
        };
        ...
        try {
          ...
          await client.graphql({
            query: updateTx.replaceAll("__typename", ""),
            variables: {
              input: {
                id: txRecord.id,
                ...modelFields,
      ...
      {...getOverrideProps(overrides, "TxUpdateForm")}
      {...rest}
    >
      <TextField
        label="Tx date"
